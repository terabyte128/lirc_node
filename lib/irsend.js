...
IRSend.prototype.send_once = function(remote, code, callback) {
	...
	if (code === 'WAKE_ON_LAN') {
		try {
			var params = require(process.env.HOME + '/.wake_on_lan.json');
			var counter = 0;

			var alarmClock = function(err, res) {
				var cb = counter++ < 5 ? alarmClock : undefined;
				console.log("waking up " + params[remote]);
				wol.wake(params[remote], cb);
			}
